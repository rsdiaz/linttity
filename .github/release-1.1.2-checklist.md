# Release 1.1.2 Checklist

This playbook prepares and publishes `linttity@1.1.2` safely.
...
2. Add a new `## [1.1.2] - YYYY-MM-DD` section in `CHANGELOG.md`.
...
```bash
git add .
git commit -m "chore(release): 1.1.2"
git tag v1.1.2
...
Expected: version `1.1.2` on `latest` tag.
